...
// Zadanie 5: greedy first-improvement LS
// Random starting solution fed in from outside; shuffles move types and positions,
// accepts the first improving move found, repeats until no improvement.
vector<int> solve_greedy_ls(vector<int> tour, const vector<vector<int>> &dist, const vector<PointData> &points) {
    int n_all = points.size();
    bool improved = true;
    ...
    while (improved) {
        ...
        vector<bool> in_t(n_all, false);
        for (int v : tour) in_t[v] = true;

        // Shuffle move type order
        int order[3] = {0, 1, 2};
        for (int i = 2; i > 0; i--) {
            int j = rand() % (i + 1);
        ...
        for (int move_type : order) {
            if (improved) break;

            if (move_type == 0 && n > 0) {
                // ADD: try each (position, outside_node) pair in random order
                vector<int> positions(n);
                iota(positions.begin(), positions.end(), 0);
                for (int i = n - 1; i > 0; i--) swap(positions[i], positions[rand() % (i + 1)]);

                vector<int> outside;
                outside.reserve(n_all - n);
                for (int v = 0; v < n_all; v++) if (!in_t[v]) outside.push_back(v);
                for (int i = (int)outside.size() - 1; i > 0; i--) swap(outside[i], outside[rand() % (i + 1)]);

                for (int pos : positions) {
                    if (improved) break;
                    int a = tour[pos], b = tour[(pos + 1) % n];
                    for (int u : outside) {
                        int delta = points[u].node_weight - (dist[a][u] + dist[u][b] - dist[a][b]);
                        if (delta > 0) {
                            tour.insert(tour.begin() + pos + 1, u);
                            improved = true;
                            break;
                        }
                    }
            ...
            } else if (move_type == 1 && n > 3) {
                // REMOVE: try each position in random order
                vector<int> positions(n);
                iota(positions.begin(), positions.end(), 0);
                for (int i = n - 1; i > 0; i--) swap(positions[i], positions[rand() % (i + 1)]);
                ...
                for (int pos : positions) {
                    ...
                    int delta = -points[u].node_weight + (dist[prev][u] + dist[u][next] - dist[prev][next]);
                    if (delta > 0) {
            ...
            } else if (move_type == 2 && n >= 4) {
                // 2-OPT: try all (i,j) pairs in random order
                ...
                for (int i = (int)pairs.size() - 1; i > 0; i--) swap(pairs[i], pairs[rand() % (i + 1)]);

                for (auto [i, j] : pairs) {
                    int a = tour[i], b = tour[(i + 1) % n], c = tour[j], d = tour[(j + 1) % n];
                    int delta = (dist[a][b] + dist[c][d]) - (dist[a][c] + dist[b][d]);
                    if (delta > 0) {
