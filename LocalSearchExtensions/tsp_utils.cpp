...
int calculate_objective(const vector<int> &cycle, const vector<vector<int>> &dist_matrix, const vector<PointData> &points) {
    ...
    for (int node_idx : cycle) {
        profit_sum += points[node_idx].node_weight;
    }

    return profit_sum - distance_sum;
}
...
void generate_svg(const string &filename, const vector<int> &cycle, const vector<PointData> &points) {
    ...
    int objective = calculate_objective(cycle, matrix, points);
...
bool verify_and_print_solution(const vector<int>& tour, const vector<vector<int>>& dist_matrix, const vector<PointData>& points, long long reported_obj, string label) {
    ...
    long long calc_profit = 0;
    long long calc_dist = 0;

    for (size_t i = 0; i < tour.size(); ++i) {
        int u = tour[i];
        int v = tour[(i + 1) % tour.size()];
        
        calc_profit += points[u].node_weight;
        calc_dist += dist_matrix[u][v];
    }
...
void save_cycle_to_file(const string &filename, const vector<int> &cycle,
    ...
    ofstream out(filename);
    if (out.is_open()) {
        int objective = calculate_objective(cycle, dist_matrix, points);
        out << objective << "\n";
