...
vector<int> solve_hae(const vector<vector<int>>& dist, const vector<PointData>& pts,
    ...
    if (g_convergence_log) {
        double tnow = chrono::duration<double, milli>(chrono::high_resolution_clock::now() - t0).count();
        (*g_convergence_log) << tnow << "," << best_obj << "\n";
    }
...
static inline vector<int> apply_ls(vector<int> tour,
                                   ...
                                   const vector<vector<int>>* nearest) {
    if (!cfg.use_ls) return tour;
    if (cfg.use_candidate_ls && nearest) {
        return solve_steepest_candidate(tour, dist, pts, *nearest);
    }
    if (cfg.use_lm_ls) {
        return solve_steepest_lm(tour, dist, pts);
    }
    return solve_steepest_baseline(tour, dist, pts);
}

static inline vector<int> init_tour(int seed_node,
                                    const vector<vector<int>>& dist,
                                    const vector<PointData>& pts,
                                    const HaeCustomCfg& cfg) {
    if (cfg.use_nn_init) {
        return solve_nn(seed_node, true, dist, pts);
    }
    return solve_random(seed_node, dist, pts);
}

vector<int> solve_hae_custom(const vector<vector<int>>& dist,
                             const vector<PointData>& pts,
    ...
    while ((int)pop.size() < P && attempts < P * 20) {
        ...
        t = apply_ls(t, dist, pts, cfg, nearest);
        int obj = calculate_objective(t, dist, pts);
        bool dup = false;
        for (auto& m : pop) if (m.obj == obj) { dup = true; break; }
        if (!dup) pop.push_back({t, obj});
    ...
    // ALNS operator weights (3 operators: op1, op2, op3)
    ...
    while (chrono::duration<double, milli>(chrono::high_resolution_clock::now() - t0).count() < time_limit_ms) {
        int pa, pb;
        if (cfg.use_pr && cfg.pr_every > 0 && (iters_done % cfg.pr_every) == 0) {
            int b1 = 0, b2 = (P > 1 ? 1 : 0);
            if (P > 1 && pop[b2].obj > pop[b1].obj) swap(b1, b2);
            for (int i = 2; i < P; i++) {
                if (pop[i].obj > pop[b1].obj) { b2 = b1; b1 = i; }
                else if (pop[i].obj > pop[b2].obj) { b2 = i; }
            }
            pa = b1; pb = b2;
            if (pa == pb) { pb = rand() % P; if (pb == pa) pb = (pa + 1) % P; }
        } else {
            pa = rand() % P;
            do { pb = rand() % P; } while (pb == pa);
        }

        int op_used = cfg.op - 1;
        if (cfg.use_alns) op_used = pick_op_alns();

        vector<int> child;
        if (op_used == 0)      child = recombine_op1(pop[pa].tour, pop[pb].tour, dist, pts);
        else if (op_used == 1) child = recombine_op2(pop[pa].tour, pop[pb].tour, dist, pts);
        else                   child = recombine_op3(pop[pa].tour, pop[pb].tour, dist, pts);

        child = apply_ls(child, dist, pts, cfg, nearest);
        int child_obj = calculate_objective(child, dist, pts);
        ...
        bool accepted = false;
        bool better_than_worst = (child_obj > pop[worst_idx].obj);
        bool sa_accept = false;
        if (!better_than_worst && cfg.use_sa_accept) {
            int delta = child_obj - pop[worst_idx].obj; // negative
            double p = exp((double)delta / T);
            if (((double)rand() / RAND_MAX) < p) sa_accept = true;
        }

        if (better_than_worst || sa_accept) {
            bool dup = false;
            for (auto& m : pop) if (m.obj == child_obj) { dup = true; break; }
            if (!dup) {
                pop[worst_idx] = {child, child_obj};
        ...
        if (cfg.use_alns) {
            used[op_used]++;
            if (accepted && child_obj == best_obj) score[op_used] += cfg.alns_reward_better;
            else if (accepted) score[op_used] += cfg.alns_reward_accepted;
            alns_step++;
            if (alns_step >= cfg.alns_window) {
                for (int i = 0; i < 3; i++) {
                    double avg = (used[i] > 0) ? (score[i] / used[i]) : 0.0;
                    w[i] = cfg.alns_decay * w[i] + (1.0 - cfg.alns_decay) * (1.0 + avg);
        ...
        // SA cooling
        if (cfg.use_sa_accept) T *= cfg.sa_alpha;

        // M4: memetic intensification
        if (cfg.use_memetic && (iters_done + 1) % cfg.memetic_every == 0) {
            // pick top-K by obj
            vector<int> idx(P);
            for (int i = 0; i < P; i++) idx[i] = i;
            partial_sort(idx.begin(), idx.begin() + cfg.memetic_top_k, idx.end(),
                         [&](int a, int b){ return pop[a].obj > pop[b].obj; });
            for (int k = 0; k < cfg.memetic_top_k; k++) {
                int i = idx[k];
                vector<int> cur = pop[i].tour;
                int cur_obj = pop[i].obj;
                for (int p = 0; p < cfg.memetic_perturb_count; p++) {
                    vector<int> t = cur;
                    perturb_ils(t, pts);
                    t = apply_ls(t, dist, pts, cfg, nearest);
                    int o = calculate_objective(t, dist, pts);
                    if (o > cur_obj) { cur = t; cur_obj = o; }
                }
                if (cur_obj > pop[i].obj) {
                    pop[i] = {cur, cur_obj};
                    if (cur_obj > best_obj) { best_obj = cur_obj; best_tour = cur; }
                }
            }
        }
...
vector<int> solve_hae_cls(const vector<vector<int>>& dist,
    ...
    // Build initial population: unique-by-objective random solutions improved by candidate LS
    int attempts = 0;
    while ((int)pop.size() < POP_SIZE && attempts < POP_SIZE * 20) {
        vector<int> t = solve_random(rand() % n_all, dist, pts);
        t = solve_steepest_candidate(t, dist, pts, nearest);
        int obj = calculate_objective(t, dist, pts);
        bool dup = false;
        for (auto& m : pop) if (m.obj == obj) { dup = true; break; }
        if (!dup) pop.push_back({t, obj});
    ...
    while (chrono::duration<double, milli>(chrono::high_resolution_clock::now() - t0).count() < time_limit_ms) {
        ...
        int pb; do { pb = rand() % POP_SIZE; } while (pb == pa);

        vector<int> child = recombine_op1(pop[pa].tour, pop[pb].tour, dist, pts);
        child = solve_steepest_candidate(child, dist, pts, nearest);
        int child_obj = calculate_objective(child, dist, pts);
